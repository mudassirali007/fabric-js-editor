...
(function () {
  ...
  var defaultButtons = [
    ...
    },
    {
      name: "icons",
      title: "Icons",
      icon: `<svg viewBox="0 0 752 752" version="1.1" xmlns="http://www.w3.org/2000/svg"><g id="#ecececff"></g><g id="#000000ff"><path fill="#000000" opacity="1.00" d=" M 370.54 91.14 C 377.71 88.66 385.89 92.98 389.08 99.62 C 484.62 265.06 580.37 430.37 676.05 595.73 C 678.49 599.53 680.35 603.95 679.79 608.56 C 679.44 617.03 671.42 624.17 662.97 623.63 C 470.96 623.68 278.95 623.65 86.94 623.65 C 80.51 624.10 73.96 620.59 71.00 614.83 C 68.83 611.38 69.37 607.06 69.78 603.21 C 71.25 598.95 73.98 595.29 76.12 591.36 C 170.10 428.54 264.15 265.76 358.16 102.96 C 360.84 97.82 364.65 92.70 370.54 91.14 Z"></path></g></svg>`
    },
    {
      name: "shapes",
      title: "Shapes",
  ...
  <div flow="down">
    <select id="selectAnimation" style="width: 200px; color:#706c6c;
    height: 35px;
    border-radius: 4px;
    padding: 5px">
      ...
      <option value="none">Animation</option>
       <option data-attribute="remove">None</option>
       
      <option value="none">---------------</option>
      <option data-attribute="dissolve">Dissolve</option>

      <option value="none">---------------</option>
      <option data-attribute="fade">Fade</option>
      
      <option value="none">---------------</option>
      <option data-attribute="scale">Scale</option>
      
      <option value="none">---------------</option>
      <option data-attribute="tiltTopV">T-top-V</option>
      
      <option value="none">---------------</option>
      <option data-attribute="tiltTopH">T-top-H</option>
      
      <option value="none">---------------</option>
      <option data-attribute="tiltBottomV">Tilt-V</option>
      
      <option value="none">---------------</option>
      <option data-attribute="tiltBottomH">Tilt-H</option>

      <option value="none">---------------</option>
      <option data-attribute="ScaleVerticalTopV">Swing</option>
      
      <option value="none">---------------</option>
      <option data-attribute="SlideInLeftV">Slide V</option>
    
      <option value="none">---------------</option>
      <option data-attribute="SlideInRightV">Slide H</option>

      <option value="none">---------------</option>
      <option data-attribute="SlideInTop">Slide Top</option>

      <option value="none">---------------</option>
      <option data-attribute="SlideInBottom">Slide Bottom</option>
    
      <option value="none">---------------</option>
      <option data-attribute="ScaleVerticalBottomV">Scale Bot</option>
    
      <option value="none">---------------</option>
      <option data-attribute="ScaleHorizontalV">Scale V</option>
      
      <option value="none">---------------</option>
      <option data-attribute="RollInLeft">Roll In Left</option>
      
      <option value="none">---------------</option>
      <option data-attribute="RollInRight">Roll In Right</option>
      
      <option value="none">---------------</option>
      <option data-attribute="RollInTop">Roll In Top</option>
      
      <option value="none">---------------</option>
      <option data-attribute="RollInBottom">Roll In Bottom</option>
  ...
  <div flow="down">
    <select id="selectTemplate">
     <option value="none">Template liste</option>
    </select>
  </div>`;
  ...
  var toolbar = function () {
    ...
    if (Array.isArray(this.buttons) && this.buttons.length) {
      ...
      defaultExtendedButtons.forEach((item) => {
        if (this.buttons.includes(item.name))
          extendedButtons.push(item);
      });
    } else {
    ...
    try {
      this.containerEl.append(`<div class="topbar" id="topbar">
         <div class="logotop">
          <a href="/index.html"><img src="TV2B Icon grafik.svg" width="50" height="50" alt="" /></a>
        </div>
        <div class="infohelpvideo">
          <i class="infovideo">?</i> 
        </div>
        <div class="guidehelp">
          <h3 class="guidetxt3">Bagskærms-guide</h3>
          <label
            ><input tooltip="Vis baggrund" flow="down"
              type="checkbox"
              id="chkbackground"
              value="supress"
              class="if-else-check-small"
            />
          </label>
        </div> 
        ${templatesList}
        <div tooltip="Download png-sekvens med alpha" flow="down" class="download">
          <a href="#0" class="previewbtn" id="previewBtn">PREVIEW</a>
          <a href="#0" class="downloadbtn" id="downloadBtn">TIL HIVE</a>
          <a href="#0" class="downloadvideobtn" id="downloadVideoBtn">web-video</a>
        </div>
        <button tooltip="Download png-fil med alpha" flow="down" class="pngDownload" style="margin-left: 24px">
          STILL
        </button>
        <input class="ifield ifield1" type="text" id="fileName" placeholder="filnavn" required />
        </div>
         <div class="toolbar" id="toolbar">
         <div class="main-buttons"></div>
         <div class="anim-container">
         ${selectorAnimation} 
         <div class="p-1"><label>delay</label><input type="range" value="0.5" min="0" max="7" step="0.5" id="anim-delay"><span class="delaytxt">0.5</span></div>
         <div class="p-1"><label>duration</label><input type="range" value="2.5" min="0" max="3" step="0.5" id="anim-duration">2.5</div>
         </div>
      ...
      (() => {
        // download buttons
        $(`${this.containerSelector} .pngDownload`).click(function () {
          const zoom = _self.canvas.getZoom();
          // toggle grid hide
          toggleGrid(_self.canvas, true);
          _self.applyZoom(1);
          downloadImage(_self.canvas.toDataURL());
          // toggle grid show
          if ($("#showGridCheckbox").is(":checked"))
            toggleGrid(_self.canvas, false);
          _self.applyZoom(zoom);
        });
        // guide buttons
        $(`${this.containerSelector} #chkbackground`).change(function () {
          if ($(this).is(":checked")) {
            _self.canvas.setOverlayImage(
              'assets/images/guide.png',
              _self.canvas.renderAll.bind(_self.canvas)
            );
          } else {
            _self.canvas.setOverlayImage(null).renderAll();
          }
        });
      ...
      // zoom
      (() => {
        let currentZoomLevel = saveInBrowser.load('canvasSetting')?.zoom ?? 1;
        ...
        $(`${this.containerSelector} .floating-zoom-level-container`).append(`
          ...
          (item) =>
            `<option value="${item}" ${item === currentZoomLevel ? "selected" : ""
            }>${item * 100}%</option>`
        )}
          </select>
        `);
        ...
        $(`${this.containerSelector} #toolbar .extended-buttons button`).click(
          function () {
            ...
            if (id === "save") {
              if (
                window.confirm(
                  "Vil du gemme lokalt på din pc?"
                )
              ) {
                saveInBrowser.save("canvasEditor", _self.canvas.toJSON(_self.propsToSave));
              }
            } else if (id === "clear") {
              if (window.confirm("Vil du slette din template?")) {
                _self.canvas.clear(), saveInBrowser.remove("canvasEditor"), saveInBrowser.remove("canvasSetting");
                $(".layer-container").html("");
              }
            } else if (id === "download") {
              $("body").append(`<div class="custom-modal-container">
              <div class="custom-modal-content">
                <div class="button-download" id="svg">Download som SVG</div>
                <div class="button-download" id="png">Download som PNG</div>
                <div class="button-download" id="jpg">Download som JPG</div>
              </div>
            </div>`);
              ...
              $("body").append(`<div class="custom-modal-container">
              <div class="custom-modal-content">
              <label for="template-name">Save template in list</label>
              <div style="display: flex">
              <input type="text" class="input-field ifield2" id="template-name" placeholder="Enter template name">
              <button  flow="down" class="save-template-btn">Save</button>
              </div>
              </div>
            </div>`);
              ...
              $(".custom-modal-container .save-template-btn").click(function () {
                _self.saveTemplate($('#template-name').val() ?? 'latest')
                $('.custom-modal-container').click();
              });
            }
          }
        );
