  ...
  var selectionSettings = function () {
    ...
    (() => {
      $(`${this.containerSelector} .toolpanel#select-panel .content`).append(`
        ...
        <textarea type="text" rows="3" class="form-control text canvas-text-edit" placeholder="Skriv tekst her..."></textarea>
          <h4>Font Style</h4>
          <div class="style">
          ...
          <div class="sizes">
            ...
            <div class="input-container"><label>Letter Spacing</label>
              <div class="custom-number-input">
              <button class="decrease">-</button>
              <input type="number" min="0" max="200" step="10" value="0" id="charSpacing">
              <button class="increase">+</button>
              </div>
            </div>
            </p>
      ...
      // Edit txt on canvas
      $(document).on('keyup input', ".canvas-text-edit", function () {
        let activeObject = _self.canvas.getActiveObject();
    ...
    (() => {
      ...
      <div class="text-background-section">
        <h4>Background</h4>
        <div class="input-container"><label>Color</label><input id="color-picker-text-background" value="black"></div>
        <div class="input-container"><label>Padding</label>
          <div class="custom-number-input">
          <button class="decrease">-</button>
          <input type="number" min="0" value="12" id="input-border-padding">
          <button class="increase">+</button>
          </div>
        </div>
        <div class="input-container"><label>Roundness</label>
        <div class="custom-number-input">
        <button class="decrease">-</button>
        <input type="number" min="1" value="1" id="input-border-roundness">
        <button class="increase">+</button>
        </div>
      </div>
        <hr>
    ...
    `);
      ...
      <div class="shadow-section">
        <div class="input-container">
          <h4>Shadow</h4>
          <input id="shadow-checkbox" type="checkbox" />
        </div>
        <div class="input-container"><label>Color</label><input id="color-picker-shadow" value="rgba(0, 0, 0, 0.65)"></div>
        <div class="input-container"><label>Blur</label>5
          <input type="range" value="5" min="0" max="100" id="shadow-blur">
        </div>
        <div class="input-container"><label>OffsetX</label>8
          <input type="range" value="8" min="-100" max="100" id="shadow-offset-x">
        </div>
        <div class="input-container"><label>OffsetY</label>10
          <input type="range" value="10" min="-100" max="100" id="shadow-offset-y">
        </div>
        <hr>
      </div>
    `);
      ...
      $(`${this.containerSelector} .toolpanel#select-panel .text-background-section #color-picker-text-background`).spectrum({
        showButtons: false,
        type: "color",
        showInput: "true",
        allowEmpty: "false",
        move: function (color) {
          let hex = 'transparent';
          color && (hex = color.toRgbString()); // #ff0000
          _self.canvas.getActiveObjects().forEach(obj => obj.set('backgroundColor', hex)); // Change background color
          _self.canvas.renderAll();
          _self.canvas.fire('object:modified');
        }
      });

      $(`${this.containerSelector} .toolpanel#select-panel .shadow-section #color-picker-shadow`).spectrum({
        showButtons: false,
        type: "color",
        showInput: "true",
        allowEmpty: "false",
        move: function (color) {
          ...
          _self.canvas.getActiveObjects().forEach(obj => {
            obj.shadow = getShadow({ ...obj.shadow, color: hex });
      ...
      $(`${this.containerSelector} .toolpanel#select-panel .text-background-section #input-border-padding`).change(function () {
        let strokeWidth = parseInt($(this).val());
        let activeObjects = _self.canvas.getActiveObjects();
      ...
      $(`${this.containerSelector} .toolpanel#select-panel .text-background-section #input-border-roundness`).change(function () {
      ...
      $(`${this.containerSelector} .toolpanel#select-panel .shadow-section #shadow-checkbox`).change(function () {
        try {
          let objectStack = $(this).is(":checked");
          let activeObjects = _self.canvas.getActiveObjects();
          activeObjects.forEach(obj => {
            obj.shadow = objectStack ? getShadow({ ...obj.shadow }) : null
          });
          _self.canvas.renderAll();
          _self.canvas.fire('object:modified');
        } catch (_) { }
      });
      $(`${this.containerSelector} .toolpanel#select-panel .shadow-section #shadow-blur`).change(function () {
        let blur = parseInt($(this).val());
        this.previousSibling.textContent = blur;
        let activeObjects = _self.canvas.getActiveObjects();
        activeObjects.forEach(obj => {
          obj.shadow = getShadow({ ...obj.shadow, blur })
        });
        _self.canvas.renderAll();
        _self.canvas.fire('object:modified');
      });

      $(`${this.containerSelector} .toolpanel#select-panel .shadow-section #shadow-offset-x`).change(function () {
        let offsetX = parseInt($(this).val());
        this.previousSibling.textContent = offsetX;
        let activeObjects = _self.canvas.getActiveObjects();
        activeObjects.forEach(obj => {
          obj.shadow = getShadow({ ...obj.shadow, offsetX })
        });
        _self.canvas.renderAll();
        _self.canvas.fire('object:modified');
      });

      $(`${this.containerSelector} .toolpanel#select-panel .shadow-section #shadow-offset-y`).change(function () {
        let offsetY = parseInt($(this).val());
        this.previousSibling.textContent = offsetY;
        let activeObjects = _self.canvas.getActiveObjects();
        activeObjects.forEach(obj => {
          obj.shadow = getShadow({ ...obj.shadow, offsetY })
        });
        _self.canvas.renderAll();
        _self.canvas.fire('object:modified');
      });
    ...
    (() => {
      ...
      $(`${this.containerSelector} .toolpanel#select-panel .fill-section #color-picker`).spectrum({
        ...
        move: function (color) {
          let hex = 'transparent';
          color && (hex = color.toRgbString()); // #ff0000
          const object = _self.canvas.getActiveObject()
          if (object.type === 'group') {
            object.getObjects().forEach(obj => obj.set('fill', hex))
    ...
    (() => {
      $(`${this.containerSelector} .toolpanel#select-panel .content`).append(`
        <div class="object-options">
          ...
          <hr>
          <label>Lock Move X Y</label>
          <div class="lock-move-xy">
            <select class="lockMovementSel" data-lock="lockMovementX" data id="custom-select">
              <option value="">---Select----</option>
              <option value="lockMovementX">Lock Move X</option>
            </select>
            <select class="lockMovementSel" data-lock="lockMovementY" id="custom-select">
              <option value="">---Select----</option>
              <option value="lockMovementY">Lock Move Y</option>
            </select>
          </div>
          <div class="origin-control-x d-flex justify-content">
            <p>Origin X :</p>
            <div class="d-flex">
            <div class="d-flex align-center ml-3 cp">
              <label for="left">Left</label><br>
              <input type="radio" id="left" name="originX" value="left">
            </div>
            <div class="d-flex align-center ml-3 cp">
              <label for="center">Center</label><br>  
              <input type="radio" id="center" name="originX" value="center">
            </div>
            <div class="d-flex align-center ml-3 cp">
              <label for="right">Right</label><br><br>
              <input type="radio" id="right" name="originX" value="right">
            </div>
            </div>
          </div>
          <div class="origin-control-y d-flex justify-content">
            <p>Origin Y :</p>
            <div class="d-flex">
            <div class="d-flex align-center ml-3 cp">
              <label for="top">Top</label><br>
              <input type="radio" id="top" name="originY" value="top">
            </div>
            <div class="d-flex align-center ml-3 cp">
              <label for="middle">Center</label><br>  
              <input type="radio" id="middle" name="originY" value="center">
            </div>
            <div class="d-flex align-center ml-3 cp">
              <label for="bottom">Bottom</label><br><br>
              <input type="radio" id="bottom" name="originY" value="bottom">
            </div>
            </div>
          </div>
        </div>
      ...
      function originXAlignment(originX) {
        let activeObj = _self.canvas.getActiveObject();
        if (!activeObj) return false;
        let canvasWidth = 1920,
          canvasHeight = 1080;
        let objectLeft = activeObj.left + (activeObj.width * activeObj.scaleX) / 2;

        activeObj.set("left", objectLeft);
        _self.canvas.renderAll()
      }
      ...
      $(document).on('change', '.origin-control-x [name="originX"]', function () {
        let value = $(this).val(),
          activeObj = _self.canvas.getActiveObject();
        setActiveProp(_self.canvas, 'originX', value);
        return false;
        if (!val) return false;
        let canvasWidth = 1920,
          canvasHeight = 1080;
        if (!activeObj) return false;
        if (val == 'left') {
          activeObj.set("left", activeObj.left + (activeObj.width * activeObj.scaleX) / 2);
          _self.canvas.renderAll();
        } else if (val == 'center') {
        }
      });

      $(document).on('change', '.origin-control-y [name="originY"]', function () {
        let value = $(this).val(),
          activeObj = _self.canvas.getActiveObject();
        setActiveProp(_self.canvas, 'originY', value);
      });

      // lock object x and y 
      $(document).on('click', ".lockMovementSel", function () {
        let val = $(this).val();
        let activeObj = _self.canvas.getActiveObject();
        if (!activeObj) return false;
        if (!val) {
          let lockMovement = $(this).data('lock');
          activeObj.set(lockMovement, false);
        } else {
          activeObj.set(val, true);
        }
      });
      ...
      $(`${this.containerSelector} .toolpanel#select-panel .object-options #delete`).click(() => {
        ...
        this.canvas.discardActiveObject().requestRenderAll(), this.canvas.fire('object:modified');
      })
    ...
    (() => {
      $(`${this.containerSelector} .toolpanel#select-panel .content`).append(`
        <div class="effect-section">
          ...
          <div class="input-container">
           <label>ClipPath</label>
           <button id="addClipPath" class="input-container">Add ClipPath</button>
          </div>
          ...
          <hr>
          <div class="gif-img-loop-controller input-container">
          <label>Loop</label>
          <input type="checkbox" class="gif-img-control" checked>
        </div>
        <hr>
        </div>
      `);
      ...
      $(`${this.containerSelector} .toolpanel#select-panel .effect-section .effect`).change(function () {
        ...
        _self.activeSelection.filters = getUpdatedFilter(currentEffect, effect, $(this).val());
