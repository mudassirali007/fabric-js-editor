    ...
    const animate = function (canvas) {
        ...
        const SlideInRight = (object) => {
            ...
            setTimeout(() => {
                // Animate position
                fabric.util.animate({
                    startValue: canvas.width + object.width, // Start outside the canvas on the right
        ...
        const SlideInTop = (object) => {
            ...
            setTimeout(() => {
                ...
                const adjustedStartValue = -(object.height * zoom);
                ...
                fabric.util.animate({
                    ...
                    duration: object.animation.duration * 1000,
                    onChange: function (value) {
                        object.set({ top: value }); // Update the top position of the object
                        canvas.renderAll(); // Render the canvas to show the updated position
                    }
                });
        ...
        const SlideInBottom = (object) => {
            const originalOpacity = object.opacity;
            object.opacity = 0;
            const zoom = canvas.getZoom();

            setTimeout(() => {
                // Calculate the adjusted startValue
                const adjustedStartValue = canvas.height + (object.height * zoom);
                // Animate position (top)
                fabric.util.animate({
                    startValue: adjustedStartValue,
                    endValue: object.top,
        ...
        const animatingObjects = () => {
            canvas.getObjects().forEach((item) => {
                ...
                const animDuration = item?.animation?.duration || 0; // Get the animation duration or default to 0
                const totalDuration = animDelay + animDuration; // Total duration for this item

                if (totalDuration > _self.maxDuration) _self.maxDuration = totalDuration; // Update maxDuration if the total duration is greater
        ...
        const downloadZip = async (progressCallback) => {
            ...
            animatingObjects();
            const animationDuration = _self.maxDuration || 0.1; // default to 2 seconds if not specified
            const totalFrames = animationDuration * fps; // Total frames for the animation
            console.log(animationDuration,totalFrames)
            for (let i = 0; i < totalFrames; i++) {
                await new Promise((resolve) => setTimeout(resolve, 1000 / fps)); // Wait for the next frame
                ...
                const percentComplete = (i / totalFrames) * 100;
            ...
            // let newFrame = newFrameCount < 40 ? 40 : newFrameCount;
        ...
        const canvasStream = canvasElement.captureStream(24); // 24 fps

        const dynamicBitrate = determineBitrate(desiredQuality);
        const downloadType = 'mp4'; //wemb
        ...
        mediaRecorder.onstop = () => {
            ...
            const blob = new Blob(recordedBlobs, { type: `video/${downloadType}` });
        ...
        const downloadVideo = async (progressCallback) => {
            ...
            const fps = 25; // 25 frames per second
            animatingObjects();
            const animationDuration = _self.maxDuration || 0.1;
            const totalFrames = animationDuration * fps; // Total frames for the animation
