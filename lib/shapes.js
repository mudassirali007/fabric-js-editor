      ...
      try {
        fabric.loadSVGFromString(
          ...
          (objects, options) => {
            ...
            if(_self.activeSelection?.id === 'ogImage'){
              obj.set({ fill: 'red' , fillRule: 'evenodd', id: 'clipPath', left: _self.activeSelection.left, top: _self.activeSelection.top});
              _self.canvas.clipPath = obj;
              _self.canvas.add(obj);
              _self.canvas.sendToBack(obj);
              _self.canvas.setActiveObject(obj).renderAll()
            } else {
              ...
              obj.set({
                left: (_self.canvas.width - obj.width) / 2,
                top: (_self.canvas.height - obj.height) / 4
              })
